...
class BreakoutGraphics:
    ...
    def __init__(self, ball_radius=BALL_RADIUS, paddle_width=PADDLE_WIDTH,
        ...
        # total score
        self.score = 0
        self.score_label = GLabel('Score: ' + str(self.score), x=0, y=self.window.height)
    ...
    def get_obj(self):
        ...
        if obj_ball_x_y is not None and obj_ball_x_y is not self.score_label:
            self.check_obj(obj_ball_x_y)
            return True
        elif obj_ball_x_2r_y is not None and obj_ball_x_2r_y is not self.score_label:
            self.check_obj(obj_ball_x_2r_y)
            return True
        elif obj_ball_x_y_2r is not None and obj_ball_x_y_2r is not self.score_label:
            self.check_obj(obj_ball_x_y_2r)
            return True
        elif obj_ball_x_2r_y_2r is not None and obj_ball_x_2r_y_2r is not self.score_label:
    ...
    # check if the object is bricks and remove
    def check_obj(self, obj):
        if obj.y < self.window.height/2:
            self.window.remove(obj)
            self.total_score()
            self.count -= 1

    # set ball position for reset_ball()
    def set_ball_position(self):
        self.ball.x = (self.window.width - self.ball.width) / 2
        self.ball.y = (self.window.height - self.ball.height) / 2

    # set ball velocity for reset_ball()
    ...
    # set start false and do set_ball_position(), set_ball_velocity() and add the ball
    def reset_ball(self):
        self.start = False
